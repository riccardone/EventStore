...
#nullable disable
				...
				yield return WithOutput($"{projection} compiles", o => {
					runner = new JintProjectionStateHandler(source, o.WriteLine, true,
				...
				for (int i = 0; i < sequences.Count; i++) {
					...
					for (int j = 0; j < sequences[i].Events.Count; j++) {
						...
						*/
						...
						if (@event.Skip) {
							...
							if (expectedDefinition.DefinesFold) {
								...
								if (!@event.InitializedPartitions.Contains(expectedPartition)) {
									yield return For($"can load current state at {er.EventNumber}@{sequence.Stream}",
										() => {
											Assert.True(partitionedState.TryGetValue(expectedPartition, out var value), $"did not find expected state for partition{expectedPartition}");
											runner.Load(value);
										});
								} else {
									...
									yield return For(
										...
										() => {
											Assert.True(runner.ProcessPartitionCreated(expectedPartition,
								...
								foreach (var expectedState in @event.ExpectedStates) {
									yield return For($"state \"{expectedState.Key}\" exists at {er.EventNumber}@{sequence.Stream}",
						...
						//var p = runner.GetStatePartition(CheckpointTag.Empty, "", e)??"";
						//runner.ProcessEvent(p, CheckpointTag.Empty, "", e, out var newState, out string newSharedState,
						//	out var envelope);
